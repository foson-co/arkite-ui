...
4. **Wide tables → `minWidth` is mandatory, and `pinned` is inert without it.** A table is `width: 100%` with auto layout, so when the columns don't fit the browser squeezes each one down to its **min-content** width *before* it ever overflows. For CJK text min-content is a single glyph, so a dense Chinese table doesn't scroll — it collapses to ~30px columns with headers stacked 3–4 lines tall (measured: header row 119px vs 34px), and `Column.pinned` never engages because the table technically "fits". Declare the width the table needs (`minWidth={960}`) and it scrolls instead; edge fades turn on automatically. **`Column.width` is a hint to the layout algorithm, not a floor** — it does not survive the squeeze, only `minWidth` does.

...
- 9eacbad: Declare `@types/react` as an optional peer dependency so React types resolve per consumer
...
- c9bdf2f: Back the React 19 half of the peer range with CI evidence.
...
- 858ca29: Pad overlays away from every screen cutout, not just the bottom one
...
- 5a2791f: Fix the saturation formula in `createTheme()`'s hex parsing. The dark branch
...
- 9a27102: Make the public-API snapshot deterministic, and stop it flaking CI.
